    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        {syncGroups.map((group) => (
          <div key={group.id} className="rounded-2xl border border-kedi-navy/10 bg-white p-5 shadow-sm dark:border-white/5 dark:bg-[#11121b]">
            ...
            <div className="mb-4 rounded-xl border border-kedi-navy/10 bg-slate-950 p-4 font-mono text-xs text-green-400 dark:border-white/5">
            ...
            <div className="flex gap-2">
              ...
              <button className="flex-1 rounded-xl border border-kedi-navy/10 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5">Cấu hình</button>
            </div>
          </div>
        ))}
      </div>
      <div className="mt-5 rounded-2xl border border-kedi-navy/10 bg-white p-5 dark:border-white/5 dark:bg-[#11121b]">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {["Mirror touch", "Paste clipboard", "Rotate screen", "Install APK"].map((action) => (
            <button key={action} className="rounded-xl border border-kedi-navy/10 px-3 py-3 text-xs font-bold text-slate-600 transition hover:border-cyan-500/40 hover:text-cyan-600 dark:border-white/10 dark:text-slate-300">
              {action}
            </button>
          ))}
        </div>
