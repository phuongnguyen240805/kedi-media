...
export default function DevicesView() {
  ...
  return (
    <>
        ...
        {["all", "online", "idle", "offline"].map((item) => (
          <button
            ...
            className={`rounded-lg border px-3 py-2 text-xs font-bold uppercase tracking-wide transition ${
              filter === item ? "border-cyan-500/40 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400" : "border-kedi-navy/10 text-slate-500 hover:border-cyan-500/30 dark:border-white/10"
            }`}
          >
            {item}
          </button>
      ...
      <div className="overflow-hidden rounded-2xl border border-kedi-navy/10 bg-white shadow-sm dark:border-white/5 dark:bg-[#11121b]">
        <div className="grid grid-cols-[1.2fr_0.7fr_0.7fr_0.7fr_0.7fr] border-b border-kedi-navy/10 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-400 dark:border-white/5">
          <span>Thiết bị</span><span>Trạng thái</span><span>Khu vực</span><span>Pin</span><span>Phiên</span>
        </div>
        {filtered.map((device) => (
          <div key={device.id} className="grid grid-cols-[1.2fr_0.7fr_0.7fr_0.7fr_0.7fr] items-center border-b border-kedi-navy/5 px-4 py-3 text-sm last:border-b-0 dark:border-white/5">
            <div>
