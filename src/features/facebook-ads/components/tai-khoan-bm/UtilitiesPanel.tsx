    ...
    <div className="p-4 flex flex-col gap-4 select-none">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-xl bg-brand-50 dark:bg-brand-950/20 text-brand-500 dark:text-brand-300 flex items-center justify-center border border-brand-50 dark:border-brand-900/30">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-1.5 bg-brand-50 hover:bg-brand-50 dark:bg-brand-950/30 dark:hover:bg-brand-900/50 text-brand-500 dark:text-brand-300 rounded-lg transition border border-brand-50/60 dark:border-brand-900/40 text-[10px] sm:text-xs font-bold cursor-pointer"
          >
            + Thêm
          </button>
          <button className="p-1.5 bg-gray-50 hover:bg-gray-100 dark:bg-gray-900 dark:hover:bg-gray-900 text-gray-400 hover:text-gray-600 dark:hover:text-white rounded-lg transition border border-gray-200 dark:border-gray-800 cursor-pointer" title="Cài đặt">
      ...
      {isAddUtilityOpen && (
        <form onSubmit={onAddUtility} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-2.5 flex items-center gap-2 animate-fade-in">
          <input
            type="text"
            placeholder="Tên tiện ích..."
            value={newUtilityName}
            onChange={(e) => setNewUtilityName(e.target.value)}
            className="flex-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg px-2.5 py-1 text-xs focus:outline-none focus:border-brand-400 transition"
          />
          <button
            type="submit"
            className="bg-brand-500 text-kedi-navy rounded-lg px-3 py-1 text-xs font-bold hover:bg-brand-600 transition cursor-pointer"
          >
            Lưu
          </button>
        </form>
      ...
      <div className="flex flex-col gap-2.5">
        {utilities.map((ut, index) => (
          <div
            key={ut.id}
            className={`flex items-center justify-between border border-gray-200 dark:border-gray-800 rounded-xl p-2 bg-gray-50/50 dark:bg-gray-900/30 hover:border-gray-200 dark:hover:border-gray-700 transition-all ${
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <button
                ...
                className={`relative inline-flex h-5.5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${
                  ut.enabled ? "bg-brand-500" : "bg-gray-200 dark:bg-gray-800"
                }`}
              >
                <span
