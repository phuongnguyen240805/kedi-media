...
export default function ConfigModal({
  ...
  return (
    <div className="fixed inset-0 z-999999 flex animate-fade-in items-center justify-center bg-kedi-navy/60 p-4 backdrop-blur-xs transition-all duration-300">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-[#11121e]">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4.5 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-brand-50 bg-brand-50 text-brand-500 dark:border-brand-900/30 dark:bg-brand-950/20 dark:text-brand-300">
          ...
          <button
            type="button"
            onClick={onClose}
            className="cursor-pointer rounded-lg p-1 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-900 dark:hover:text-white"
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
        ...
        <div className="flex max-h-[60vh] flex-col gap-5 overflow-y-auto p-6 text-left">
                ...
                <button
                  ...
                  className={`flex cursor-pointer flex-col rounded-xl border p-4.5 text-left transition-all ${
                    sourceType === source.id
                      ? "border-brand-400 bg-brand-50/40 text-brand-600 shadow-sm dark:border-brand-400 dark:bg-brand-950/10 dark:text-brand-300"
                      : "border-gray-200 bg-gray-50/50 hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900/50 dark:hover:bg-gray-900"
                  }`}
                >
                  <div className="mb-1 flex w-full items-center justify-between">
                    <span className="text-xs font-bold sm:text-sm">{source.title}</span>
                    {sourceType === source.id && (
                      <span className="flex h-4 w-4 items-center justify-center rounded-full bg-brand-400 text-white">
          ...
          <div>
            <div className="mb-2.5 flex select-none items-center gap-2.5">
              <input
                ...
                className="h-4.5 w-4.5 cursor-pointer rounded text-brand-500 focus:ring-brand-400"
              />
            ...
            {advancedConfig && (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                {dataOptionItems.map((option) => (
                  <label
                    key={option.key}
                    className="flex cursor-pointer select-none items-start gap-2.5 rounded-xl border border-gray-200 bg-gray-50/50 p-3 transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900/40 dark:hover:bg-gray-900"
                  >
                    <input
                      type="checkbox"
                      ...
                      className="mt-0.5 h-4 w-4 cursor-pointer rounded text-brand-500 focus:ring-brand-400"
                    />
                    <div className="flex flex-col text-left">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-gray-200 bg-gray-50/50 p-4.5 dark:border-gray-800 dark:bg-gray-900/30">
            <div className="flex flex-col text-left">
            ...
            <input
              ...
              className="w-24 rounded-xl border border-gray-200 bg-white px-3 py-2 text-center text-xs font-semibold focus:border-brand-400 focus:outline-none dark:border-gray-800 dark:bg-gray-900"
            />
          </div>
        </div>

        <div className="flex items-center justify-end gap-3.5 border-t border-gray-200 bg-gray-50 px-6 py-4.5 dark:border-gray-800 dark:bg-gray-900/60">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            type="button"
            onClick={onLoadData}
            className="flex cursor-pointer select-none items-center gap-2 rounded-xl bg-brand-500 px-5 py-2.5 text-xs font-bold text-kedi-navy shadow-xs transition hover:bg-brand-600"
          >
