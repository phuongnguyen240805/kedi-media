  ...
  if (!dataLoaded) {
    return (
      <div className="flex min-h-[500px] flex-col items-center justify-center p-12 text-center">
        <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-brand-50 bg-brand-50 text-brand-500 dark:border-brand-900/30 dark:bg-brand-950/20 dark:text-brand-300">
        ...
        <button
          type="button"
          onClick={openConfig}
          className="flex cursor-pointer select-none items-center gap-2 rounded-xl bg-brand-500 px-6 py-3 text-xs font-bold text-kedi-navy shadow-xs transition hover:scale-[1.02] hover:bg-brand-600 active:scale-[0.98]"
        >
          Tải dữ liệu
        </button>
      </div>
    ...
    <div className="flex flex-1 flex-col justify-between">
      <div className="w-full overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tr>
              <th scope="col" className="w-10 px-4 py-3.5 text-center">
                <input
                  ...
                  className="h-4 w-4 cursor-pointer rounded text-brand-500 focus:ring-brand-400"
                />
              </th>
          ...
          <tbody className="divide-y divide-gray-200 text-xs text-gray-700 dark:divide-gray-800 dark:text-gray-300">
                      ...
                      <div className="flex flex-col text-left">
                        <span className="flex items-center gap-1.5 font-semibold text-gray-900 dark:text-white">
                          ...
                          {page.verification === "BLUE" ? (
                            <span className="rounded border border-brand-100 bg-brand-50 px-1 text-[9px] font-extrabold text-brand-600 dark:border-brand-900/30 dark:bg-brand-950/40 dark:text-brand-300">Tích xanh</span>
                          ) : page.verification === "GRAY" ? (
                            <span className="rounded border border-gray-200 bg-gray-50 px-1 text-[9px] font-extrabold text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">Tích xám</span>
                          ) : null}
                        </span>
      ...
      <div className="flex select-none items-center justify-between border-t border-gray-200 bg-gray-50/50 px-5 py-3.5 text-xs font-semibold text-gray-500 dark:border-gray-800 dark:bg-gray-900/30 dark:text-gray-400">
        <div className="flex gap-4">
          <span>TỔNG: <strong className="text-gray-700 dark:text-white">{filteredPages.length}</strong></span>
          <span>CHỌN: <strong className="text-brand-500 dark:text-brand-300">{selectedIds.length}</strong></span>
          <span>VÙNG: <strong className="text-gray-700 dark:text-white">{selectedRegion}</strong></span>
        </div>
        <button
          type="button"
          onClick={openConfig}
          className="flex cursor-pointer items-center gap-1 font-bold text-brand-500 transition hover:text-brand-600 dark:text-brand-300 dark:hover:text-brand-200"
        >
