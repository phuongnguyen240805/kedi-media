...
export default function AppStorePage() {
  ...
  const selectedAppDetails = selectedApp
    ? appDetailContent[selectedApp.id] ?? {
        ...
        integrations: selectedApp.tags ?? ["Kedi", "Automation"],
      }
    : null;
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-[#11121e]">
          <div className="flex flex-col gap-5 p-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-brand-50 text-2xl font-black text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedAppDetails.integrations.map((integration) => (
                <span key={integration} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 dark:border-gray-800 dark:text-gray-300">
                  <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-50 text-xs font-black text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`pb-3 px-3 relative font-bold transition-all duration-150 flex items-center gap-1.5 cursor-pointer hover:text-kedi-navy dark:hover:text-kedi-yellow ${
                  isActive
              ...
              >
                ...
                {isActive && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-kedi-yellow dark:bg-kedi-yellow animate-fade-in" />
                )}
              </button>
            );
          })}
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/40 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl dark:border-gray-800 dark:bg-[#11121e]">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <div className="mb-2 text-[10px] font-black uppercase tracking-wider text-kedi-navy dark:text-kedi-yellow">
                  Chi tiết sản phẩm
                </div>
            ...
            {selectedApp.tags && selectedApp.tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedApp.tags.map((tag) => (
                  <span key={tag} className="rounded-lg bg-brand-50 px-2.5 py-1 text-xs font-bold text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow">
                    {tag}
                  </span>
                ))}
              </div>
