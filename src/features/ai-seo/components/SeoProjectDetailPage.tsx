  ...
  if (projectQuery.isError || !projectQuery.data) {
    return (
      <div className="mx-auto max-w-6xl p-6 text-center">
        ...
        <Link href="/ai-seo" className="mt-3 inline-block text-xs font-semibold text-kedi-navy">
          Quay lại dashboard
        </Link>
      </div>
    )
        ...
        <div className="mx-auto max-w-7xl px-4 py-5 md:px-8">
          ...
          <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow">
          ...
          {rows.slice(0, 8).map((row) => (
            <div key={row.x} className="grid grid-cols-[120px_1fr_48px] items-center gap-3">
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                <div className="h-full rounded-full bg-kedi-yellow" style={{ width: `${Math.max(4, (row.y / max) * 100)}%` }} />
              </div>
