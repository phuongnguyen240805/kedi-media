    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-slate-950 px-6 text-center text-slate-100">
      ...
      {!error ? (
        ...
        <>
          ...
          <button
            type="button"
            className="rounded-md bg-kedi-yellow px-3 py-1.5 text-sm font-semibold text-slate-950"
            onClick={() => {
              started.current = false;
              setError(null);
      ...
      {target ? (
        <a href={target} className="text-xs text-kedi-yellow/80 underline break-all">
          Continue
        </a>
      ) : null}
