              ...
              {days.map((d, i) => {
                ...
                return (
                  <th key={d} className="px-2 py-3 text-center">
                    <span className={`block text-xs font-bold uppercase ${isToday ? "text-kedi-navy dark:text-kedi-yellow" : "text-gray-500 dark:text-gray-400"}`}>{d}</span>
                    <span className={`block text-[10px] ${isToday ? "text-kedi-navy" : "text-gray-400"}`}>{date.toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit" })}</span>
                  </th>
                );
              })}
            </tr>
