              ...
              {shippingEnabled ? (
                shippingIntegrations.length ? (
                  <div className="space-y-3">
                    ...
                    <button type="button" disabled={shippingBusy || !shippingAddress.trim() || !province || !district || !ward || (["ghn", "viettel_post"].includes(shippingProvider) && (!districtId || !wardCode)) || (shippingProvider === "ghn" && !serviceId && !serviceTypeId)} onClick={() => void calculateShippingFee()} className="h-9 w-full rounded-lg border border-kedi-yellow text-xs font-semibold text-[#3a5680] transition hover:bg-brand-50 disabled:cursor-not-allowed disabled:opacity-40 dark:text-[#ffe08a] dark:hover:bg-brand-500/10">
