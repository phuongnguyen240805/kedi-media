        ...
        <form onSubmit={handleSave} className="px-6 pb-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 text-xs rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-slate-800 dark:text-gray-100 focus:outline-none focus:border-brand-400 font-medium font-mono"
            />
...
export const ProductCustomFields: React.FC = () => {
  ...
  const getTypeStyle = (type: DataType) => {
    const map: Record<DataType, string> = {
      "Dòng văn bản":  "text-brand-700 bg-brand-100 dark:text-[#ffe08a] dark:bg-kedi-yellow/10",
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-900 bg-gray-50/50 dark:bg-gray-800/10">
              <th className="py-3.5 px-5 text-xs font-bold text-slate-900 dark:text-slate-200">Tên hiển thị</th>
              <th className="py-3.5 px-5 text-xs font-bold text-slate-900 dark:text-slate-200">Tên trường</th>
              <th className="py-3.5 px-5 text-xs font-bold text-slate-900 dark:text-slate-200">Loại dữ liệu</th>
              <th className="py-3.5 px-5 text-xs font-bold text-slate-900 dark:text-slate-200">Ngày cập nhật</th>
              ...
              filtered.map((f) => (
                <tr key={f.id} className="hover:bg-slate-50/40 dark:hover:bg-gray-800/10 transition">
                  ...
                  <td className="py-4 px-5">
                    <code className="text-ui-caption font-mono font-bold text-brand-500 dark:text-brand-400 bg-brand-50 dark:bg-brand-950/30 px-2 py-0.5 rounded">{f.fieldName}</code>
                  </td>
                  <td className="py-4 px-5">
