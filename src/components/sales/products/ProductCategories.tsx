        ...
        <form onSubmit={handleSave} className="px-6 pb-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-stretch gap-3">
              {/* Drop zone */}
              <div className="w-20 h-[72px] border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg flex flex-col items-center justify-center gap-1 cursor-pointer hover:border-brand-400 hover:bg-brand-50/40 dark:hover:bg-brand-950/20 transition select-none text-slate-400 flex-shrink-0">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-5.5 w-10 items-center rounded-full transition-colors cursor-pointer ${visible ? "bg-kedi-yellow" : "bg-gray-300 dark:bg-gray-600"}`}
            >
  ...
  return (
    ...
    <div className="space-y-5 flex-1">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 dark:border-gray-900 pb-5">
        ...
        <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-[#3a5680] bg-kedi-yellow hover:bg-[#e0a800] rounded-lg shadow-sm transition cursor-pointer whitespace-nowrap">
          + Tạo danh mục mới
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-900 bg-gray-50/50 dark:bg-gray-800/10">
              ...
              <th className="py-3.5 px-4 text-xs font-bold text-slate-900 dark:text-slate-200">Tên danh mục</th>
              <th className="py-3.5 px-4 text-xs font-bold text-slate-900 dark:text-slate-200">Số sản phẩm</th>
              <th className="py-3.5 px-4 text-xs font-bold text-slate-900 dark:text-slate-200">Hiển thị danh mục</th>
                  ...
                  <tr key={cat.id} className={`hover:bg-slate-50/40 dark:hover:bg-gray-800/10 transition ${isSelected ? "bg-[#f4f7ff] dark:bg-brand-950/10" : ""}`}>
                    ...
                    <td className="py-4 px-4">
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors cursor-pointer ${cat.visible ? "bg-kedi-yellow" : "bg-gray-300 dark:bg-gray-600"}`}
                      >
              ...
              <tr>
                <td colSpan={5} className="py-24 text-center select-none">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    {!searchQuery && (
                      <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-[#3a5680] bg-kedi-yellow hover:bg-[#e0a800] rounded-lg shadow-sm transition cursor-pointer">
                        + Tạo danh mục mới
                      </button>
                    )}
                  </div>
