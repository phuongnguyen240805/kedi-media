  ...
  return (
    <div className="space-y-6 flex-1">
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-xs font-bold transition-all relative border-b-2 rounded-t-lg cursor-pointer ${
                  isActive
                    ? "border-kedi-yellow text-kedi-navy bg-brand-50/40 dark:bg-kedi-yellow/10"
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center justify-between p-4 bg-brand-50/60 dark:bg-kedi-yellow/10 border border-kedi-yellow/20 dark:border-kedi-yellow/30 rounded-xl animate-fade-in select-none">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-800 dark:text-slate-200">
            <span>Đã chọn <strong className="text-kedi-navy dark:text-kedi-yellow">{selectedIds.length}</strong> khách hàng</span>
          </div>
                        ...
                        <div className="flex flex-wrap gap-1 max-w-[150px]">
                          {item.tags.length > 0 ? (
                            item.tags.map((tag, idx) => (
                              <span
                                key={idx}
                                className="px-2 py-0.5 text-ui-nano font-bold text-kedi-navy bg-brand-50 dark:text-kedi-yellow dark:bg-kedi-yellow/10 rounded-full"
                              >
                                {tag}
                              </span>
                            ))
