    ...
    <ApiState isLoading={isLoading} error={error}>
      <div className="space-y-6 flex-1">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          {showDateRange && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-50 dark:bg-kedi-yellow/10 text-kedi-navy dark:text-kedi-yellow border border-kedi-yellow/30 dark:border-kedi-yellow/30 rounded-lg text-xs font-bold">
              <span>📅 Khoảng thời gian: 14/05/2026 – 13/06/2026</span>
              <button
                onClick={() => setShowDateRange(false)}
                className="text-kedi-yellow hover:text-kedi-navy p-0.5 hover:bg-brand-50 dark:hover:bg-kedi-yellow/10 rounded-full transition cursor-pointer"
              >
                <IconX size={12} />
              </button>
            </div>
