...
export const IntegrationList: React.FC<IntegrationListProps> = ({ isSimulated }) => {
  ...
  const renderPlatformLogo = (type: string) => {
    switch (type) {
      case "Ecommerce":
        return (
          <div className="w-5 h-5 rounded-full bg-brand-50 dark:bg-kedi-yellow/10 text-kedi-navy flex items-center justify-center flex-shrink-0">
    ...
    <div className="flex-1 flex flex-col space-y-5">
      ...
      {subTab === "library" ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {[
            { name: "Zalo OA", desc: "Gửi tin nhắn chăm sóc khách hàng bằng kênh Zalo ZNS chính thống.", icon: "Z", color: "bg-kedi-yellow" },
