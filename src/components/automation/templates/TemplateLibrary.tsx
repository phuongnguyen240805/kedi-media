...
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({ onSelectTemplate }) => {
  ...
  const getTagStyle = (tag: string) => {
    ...
    if (t.includes("zalo")) return "bg-brand-50 text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow";
  ...
  const renderChannelIcon = (tag: string) => {
    ...
    if (t.includes("facebook")) return <IconFacebook size={16} className="inline mr-1 text-kedi-navy" />;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {filteredTemplates.map((item) => (
              <div
                ...
                className="group flex flex-col justify-between p-5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-theme-xs hover:shadow-md dark:hover:border-kedi-yellow/40/50 hover:border-kedi-yellow/50 transition duration-300 cursor-pointer"
              >
                <div>
                  {/* Category & Recommends badge */}
                  <div className="flex items-center justify-between mb-3.5">
                    <span className="text-ui-micro font-extrabold text-kedi-navy dark:text-kedi-yellow tracking-wider uppercase bg-brand-50 dark:bg-kedi-yellow/10 px-2 py-0.5 rounded">
                      {item.category}
                    </span>
                    {item.recommends && (
                  ...
                  {/* Title */}
                  <h3 className="text-sm font-bold text-slate-800 dark:text-white group-hover:text-kedi-navy dark:group-hover:text-kedi-yellow transition mb-2 line-clamp-2">
                    {item.name}
                  </h3>
