    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        <input
          ...
          value={value}
          readOnly
          className="w-full cursor-default rounded-xl border border-gray-200 bg-gray-50 py-2 pl-4 pr-10 text-xs font-semibold text-gray-800 shadow-theme-xs transition-all focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-200"
        />
        <button
          type="button"
          onClick={() => onToggleVisibility(field.key)}
          aria-label={isVisible ? `Ẩn ${field.label}` : `Hiện ${field.label}`}
          className="absolute right-3 top-2.5 cursor-pointer text-gray-400 transition hover:text-brand-400 dark:hover:text-brand-300"
        >
