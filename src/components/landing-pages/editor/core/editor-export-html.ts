...
function renderBlockHtml(block: EditorBlock): string {
  ...
  switch (b.type) {
    ...
    case "button":
      return `<div ${attrs} id="${b.id}" class="lp-frame" style="display:flex;justify-content:${str(props.align, "center")};align-items:center;width:100%;height:100%;"><a href="${escapeAttr(str(props.url, "#"))}" style="display:inline-flex;width:${props.fullWidth ? "100%" : "auto"};justify-content:center;padding:${buttonPadding(str(props.size, "md"))};border-radius:${num(props.borderRadius, 8)}px;background:${props.style === "filled" ? str(props.color, "#FFC629") : "transparent"};border:${props.style === "ghost" ? "2px solid transparent" : `2px solid ${str(props.color, "#FFC629")}`};color:${props.style === "filled" ? str(props.textColor, "#ffffff") : str(props.color, "#FFC629")};text-decoration:none;font-weight:700;align-items:center;">${escapeHtml(str(props.label, "Button"))}</a></div>`;
    case "spacer":
    ...
    case "feature_card":
      return `<div ${attrs} id="${b.id}" class="lp-frame" style="background:${str(props.bgColor, "#ffffff")};border:1px solid ${str(props.borderColor, "#e5e7eb")};border-radius:${num(props.borderRadius, 12)}px;padding:20px;display:flex;flex-direction:column;justify-content:center;width:100%;height:100%;box-sizing:border-box;"><div style="width:40px;height:40px;border-radius:10px;background:${str(props.iconBg, "#dbeafe")};color:${str(props.iconColor, "#FFC629")};display:flex;align-items:center;justify-content:center;font-size:20px;margin-bottom:12px;">${escapeHtml(str(props.icon))}</div><h3 style="margin:0 0 8px;color:#0f172a;font-size:18px;">${escapeHtml(str(props.title))}</h3><p style="margin:0;color:#475569;font-size:13px;line-height:1.6;">${escapeHtml(str(props.description))}</p></div>`;
    case "testimonial":
...
function renderFunnelPopupHtml(attrs: string, props: Record<string, unknown>, id: string): string {
  const accent = str(props.accentColor, "#FFC629");
