    ...
    <div className="flex h-full min-h-0 bg-[#f4f7f5] text-slate-900 dark:bg-[#101412] dark:text-slate-100">
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${
                  isActive
                    ? "bg-kedi-yellow/15 font-semibold text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow"
              ...
              >
                <Icon className={`h-[18px] w-[18px] ${isActive ? "text-kedi-navy dark:text-kedi-yellow" : "text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200"}`} />
                <span>{item.label}</span>
                {isActive && <span className="ml-auto h-2 w-2 rounded-full bg-kedi-yellow" />}
              </button>
            );
          })}
        </nav>
        <div className="mt-auto rounded-xl border border-kedi-yellow/40 bg-brand-50 p-3 dark:border-kedi-yellow/15 dark:bg-kedi-yellow/[0.06]">
          <div className="flex items-center gap-2 text-xs font-semibold text-kedi-navy dark:text-kedi-yellow"><Headphones className="h-4 w-4" /> Phân tích CSKH</div>
          <p className="mt-2 text-[10px] leading-4 text-kedi-navy/70 dark:text-kedi-yellow/60">Theo dõi hiệu suất xử lý và chất lượng chăm sóc khách hàng.</p>
        </div>
      </aside>
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        <div className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur dark:border-white/[0.07] dark:bg-[#121715]/95 lg:hidden">
          ...
          >
            <span className="flex items-center gap-2 text-sm font-semibold"><CurrentMobileIcon className="h-4 w-4 text-kedi-navy" />{currentNavigationItem.label}</span>
          ...
          {isMobileNavigationOpen && (
            <div className="absolute left-4 right-4 top-[68px] rounded-xl border border-slate-200 bg-white p-2 shadow-2xl dark:border-white/10 dark:bg-[#171c1a]">
              {statisticNavigationItems.map((item) => {
                const Icon = item.icon;
                return <button key={item.id} onClick={() => handleStatisticViewChange(item.id)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm ${item.id === currentView ? "bg-kedi-yellow/15 text-kedi-navy dark:text-kedi-yellow" : "text-slate-600 dark:text-slate-300"}`}><Icon className="h-4 w-4" />{item.label}</button>;
              })}
            </div>
          )}
        </div>
