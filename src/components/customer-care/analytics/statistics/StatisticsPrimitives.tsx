...
export const ladiChartColors = ["#FFC629", "#10b981", "#06b6d4", "#f59e0b", "#ec4899"];
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="inline-flex h-10 min-w-32 items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm hover:border-kedi-yellow dark:border-white/10 dark:bg-[#151a18] dark:text-slate-200"
      >
          ...
          {(Object.entries(statisticRangeLabels) as Array<[StatisticRangePreset, string]>).map(([range, label]) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition ${
                range === value
                  ? "bg-kedi-yellow/15 font-semibold text-kedi-navy dark:text-kedi-yellow"
                  : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/5"
              }`}
            >
              {label}
              {range === value && <span className="h-2 w-2 rounded-full bg-kedi-yellow" />}
            </button>
          ))}
        </div>
      )}
    ...
    <div className={`flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-2 dark:border-white/[0.08] dark:bg-[#151a18] ${compact ? "mb-4" : "mb-5"}`}>
      ...
      {showSearch && (
        <label className="relative min-w-48 flex-1">
          ...
          <input
            className="h-9 w-full rounded-lg border border-slate-200 bg-transparent pl-9 pr-3 text-xs outline-none placeholder:text-slate-400 focus:border-kedi-yellow dark:border-white/10"
            placeholder="Tìm kiếm"
          />
        </label>
      )}
      <button className="ml-auto inline-flex h-9 items-center gap-2 rounded-lg bg-kedi-yellow px-3 text-xs font-semibold text-slate-950 hover:bg-kedi-yellow">
...
export function MetricTile({
  ...
  const tones = {
    lime: "bg-kedi-yellow/10 text-kedi-navy dark:text-kedi-yellow",
...
export function EmptyStatisticsState({
  ...
  return (
    <div className="flex min-h-52 flex-col items-center justify-center text-center">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-kedi-yellow/10 text-kedi-navy">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs">
          ...
          <tbody className="divide-y divide-slate-100 bg-white text-slate-700 dark:divide-white/[0.06] dark:bg-[#151a18] dark:text-slate-300">
            {rows.map((row, index) => (
              <tr key={index} className="transition hover:bg-brand-50/40 dark:hover:bg-kedi-yellow/[0.04]">
                {columns.map((column) => (
                  <td
                    key={column.key}
      ...
      {items.map((item) => (
        <button
          ...
          className={`rounded-md px-3 py-1.5 text-xs font-medium transition ${
            value === item.value
              ? "bg-white text-kedi-navy shadow-sm dark:bg-[#232a26] dark:text-kedi-yellow"
