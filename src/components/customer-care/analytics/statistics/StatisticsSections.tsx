    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <StatisticsCard
          ...
          action={<button className="text-xs font-semibold text-kedi-navy dark:text-kedi-yellow">Xem thống kê tương tác</button>}
        >
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3">
        <StatisticsCard title="Đánh giá" description="Đánh giá của khách hàng về nhân viên" action={<ChevronRight className="h-5 w-5 text-slate-400" />}>
          ...
          <div className="mt-4 space-y-2">
            <LegendLine color="#FFC629" label="Tốt (5 sao)" value="82%" />
        ...
        <StatisticsCard title="Tổng đài" description="Tổng số lượng cuộc gọi trên tổng đài" action={<ChevronRight className="h-5 w-5 text-slate-400" />}>
          ...
          <div className="mt-4 space-y-2">
            <LegendLine color="#FFC629" label="Cuộc gọi đến" value="412" />
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        <StatisticsCard title="Top thẻ hội thoại" description="Các thẻ đang được gắn nhiều nhất" action={<ChevronRight className="h-5 w-5 text-slate-400" />}>
          <div className="space-y-1">
            {tagStatisticRows.slice(0, 10).map((tag) => (
              <div key={tag.id} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-kedi-yellow/[0.06]">
        ...
        <StatisticsCard
          ...
          action={<button className="text-xs font-semibold text-kedi-navy dark:text-kedi-yellow">Xem thống kê nhân viên</button>}
        >
            ...
            {userStatisticRows.slice(0, 5).map((user, index) => (
              <div key={user.id} className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 dark:border-white/[0.08]">
                <div className="relative">
                  ...
                  <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-kedi-yellow text-[10px] font-bold text-slate-950">{index + 1}</span>
                </div>
...
export function PageStatistic() {
  ...
  const rows = pageStatisticRows.map((row) => ({
    ...
    rate: <span className="font-semibold text-kedi-navy dark:text-kedi-yellow">{row.responseRate}%</span>,
    orders: row.orders,
  }));
...
export function UserStatistic() {
  ...
  const rows = userStatisticRows.map((user, index) => ({
    rank: <span className="flex h-6 w-6 items-center justify-center rounded-full bg-kedi-yellow/15 font-semibold text-kedi-navy dark:text-kedi-yellow">{index + 1}</span>,
    employee: (
    ...
    rate: <span className="text-kedi-navy dark:text-kedi-yellow">{user.responseRate}%</span>,
    ...
    <div>
      ...
      <div className="mt-4">
        <StatisticsCard title="Tương tác theo thời gian" description="Chi tiết tương tác trong khoảng thời gian đã chọn">
          <StatisticsChart
            options={baseCartesianOptions(activityOverviewLabels, { colors: ["#FFC629", "#06b6d4"], fill: { type: "gradient", gradient: { opacityFrom: 0.15, opacityTo: 0.01 } } })}
            series={[
              ...
              {tagStatisticRows.slice(0, 6).map((tag) => (
                <div key={tag.id}>
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-white/[0.06]">
                    <div className="h-full rounded-full bg-kedi-yellow" style={{ width: `${Math.max(14, (tag.conversations / tagStatisticRows[0].conversations) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <StatisticsCard title="Biểu đồ cuộc gọi theo thời gian" description="Số cuộc gọi đến và đi theo từng ngày">
          <StatisticsChart
            options={baseCartesianOptions(callCenterStatisticRows.slice().reverse().map((row) => row.date.slice(0, 5)), { colors: ["#FFC629", "#06b6d4"], fill: { type: "gradient", gradient: { opacityFrom: 0.18, opacityTo: 0.02 } } })}
            series={[
        ...
        <StatisticsCard title="Phân bổ trạng thái" description="Tỷ lệ cuộc gọi được trả lời và bỏ lỡ">
          <StatisticsChart
            options={{ chart: { type: "donut", fontFamily: "Arial, Helvetica, sans-serif" }, colors: ["#FFC629", "#ef4444"], labels: ["Đã trả lời", "Bỏ lỡ"], dataLabels: { enabled: false }, stroke: { width: 0 }, legend: { position: "bottom", labels: { colors: "#cbd5e1" } }, plotOptions: { pie: { donut: { size: "72%", labels: { show: true, total: { show: true, label: "Tổng cuộc gọi", color: "#94a3b8", formatter: () => "624" }, value: { color: "#f8fafc" } } } } } }}
            series={[567, 57]}
            type="donut"
            height={290}
          />
...
export function AdsStatistic() {
  ...
  const rows = adsStatisticRows.map((row) => ({
    ...
    conversion: <span className="font-semibold text-kedi-navy dark:text-kedi-yellow">{row.conversionRate}%</span>,
  }));
  return (
    <div>
      ...
      <div className="mb-4 overflow-hidden rounded-xl border border-kedi-yellow/50 bg-gradient-to-r from-lime-500/20 via-emerald-500/10 to-cyan-500/10 px-5 py-4 dark:border-kedi-yellow/20">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><div className="text-sm font-semibold text-slate-900 dark:text-white">Tối ưu tin nhắn quảng cáo cùng Kedi</div><div className="mt-1 text-xs text-slate-500 dark:text-slate-300">Theo dõi hiệu quả chiến dịch, hội thoại và đơn hàng trên cùng một báo cáo.</div></div>
          </div>
          <button className="rounded-lg bg-kedi-yellow px-4 py-2 text-xs font-semibold text-slate-950 hover:bg-kedi-yellow">Mở trung tâm quảng cáo</button>
        </div>
      </div>
      <DateFilterBar />
      ...
      <div className="mt-4">
        <StatisticsCard title="Biểu đồ hiệu quả theo thời gian" description="Tin nhắn, khách hàng và đơn hàng được tạo từ quảng cáo">
          <StatisticsChart
            options={baseCartesianOptions(adsStatisticRows.slice().reverse().map((row) => row.date.slice(0, 5)), { colors: ["#FFC629", "#06b6d4", "#f59e0b"], fill: { type: "gradient", gradient: { opacityFrom: 0.2, opacityTo: 0.02 } } })}
            series={[
...
export function FeedbackStatistic() {
  ...
  const rows = feedbackStatisticRows.map((row) => ({
    date: <span className="font-medium text-slate-800 dark:text-slate-200">{row.date}</span>,
    total: row.total,
    positive: <span className="text-kedi-navy dark:text-kedi-yellow">{row.positive}</span>,
    ...
    <div>
      ...
      <div className="grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
        <StatisticsCard title="Đánh giá" description="Đánh giá của khách hàng về nhân viên">
          <StatisticsChart
            options={{ chart: { type: "donut", fontFamily: "Arial, Helvetica, sans-serif" }, colors: ["#FFC629", "#f59e0b", "#ef4444"], labels: ["Tốt (5 sao)", "Trung bình (3 - 4 sao)", "Cần cải thiện (1 - 2 sao)"], dataLabels: { enabled: false }, stroke: { width: 0 }, legend: { position: "bottom", labels: { colors: "#cbd5e1" } }, plotOptions: { pie: { donut: { size: "74%", labels: { show: true, total: { show: true, label: "Trung bình", color: "#94a3b8", formatter: () => "4.6 ★" }, value: { color: "#f8fafc" } } } } } }}
  ...
  return (
    <div>
      <StatisticPageHeader
        ...
        actions={<button className="inline-flex h-10 items-center gap-2 rounded-lg bg-kedi-yellow px-4 text-xs font-semibold text-slate-950"><Save className="h-4 w-4" />Tạo bản sao lưu</button>}
      />
      ...
      >
        ...
        <div className="mt-4">
          <DataTable
            ...
            rows={exportJobs.map((exportJob) => ({ ...exportJob, status: <span className="ladi-status-badge inline-flex items-center rounded-full bg-kedi-yellow/10 px-2 py-1 text-kedi-navy dark:text-kedi-yellow">{exportJob.status}</span> }))}
          />
        </div>
      </StatisticsCard>
    </div>
...
function OverviewChannelRow({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number; color: "lime" | "cyan" }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 dark:border-white/[0.08]">
      <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${color === "lime" ? "bg-kedi-yellow/10 text-kedi-navy" : "bg-cyan-500/10 text-cyan-500"} [&>svg]:h-5 [&>svg]:w-5`}>{icon}</div>
...
function CompactKpi({ label, value, color }: { label: string; value: string; color: "lime" | "cyan" | "amber" | "emerald" }) {
  const colors = { lime: "bg-kedi-yellow", cyan: "bg-cyan-500", amber: "bg-amber-500", emerald: "bg-emerald-500" };
...
function RingSummary({ value, label, suffix }: { value: string; label: string; suffix?: React.ReactNode }) {
  return (
    <div className="mx-auto flex h-52 w-52 items-center justify-center rounded-full border-[22px] border-slate-100 dark:border-white/[0.04]">
      <div className="text-center"><div className="text-xs text-slate-500 dark:text-slate-400">{label}</div><div className="mt-2 flex items-center justify-center gap-2 text-3xl font-medium text-slate-950 dark:text-white">{value}{suffix}</div><div className="mt-3 inline-flex items-center gap-1 rounded-md bg-kedi-yellow/15 px-2 py-1 text-[11px] font-semibold text-kedi-navy dark:text-kedi-yellow"><ArrowUpRight className="h-3 w-3" /> 8%</div></div>
    </div>
  );
}
...
function UserRankRow({ user, index, metric }: { user: (typeof userStatisticRows)[number]; index: number; metric: string }) {
  return <div className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 dark:border-white/[0.08]"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-kedi-yellow/15 text-xs font-semibold text-kedi-navy dark:text-kedi-yellow">{index + 1}</span><img src={user.avatar} alt="" className="h-10 w-10 rounded-full object-cover" /><div className="min-w-0 flex-1"><div className="truncate text-sm font-medium text-slate-800 dark:text-white">{user.name}</div><div className="mt-0.5 text-[11px] text-slate-400">Phản hồi {user.responseRate}%</div></div><span className="text-xs font-semibold text-slate-700 dark:text-slate-200">{metric}</span></div>;
}

function ChangeValue({ value }: { value: number }) {
  const positive = value >= 0;
  return <span className={`inline-flex items-center gap-1 font-semibold ${positive ? "text-kedi-navy dark:text-kedi-yellow" : "text-rose-500"}`}>{positive ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}{Math.abs(value)}%</span>;
}

function TableFooter() {
  return <div className="flex items-center justify-between text-xs text-slate-400"><span>Hiển thị 1 - 7 trên 30 kết quả</span><div className="flex gap-1"><button className="rounded-md border border-slate-200 px-2 py-1 dark:border-white/10">Trước</button><button className="rounded-md bg-kedi-yellow px-2 py-1 font-semibold text-slate-950">1</button><button className="rounded-md border border-slate-200 px-2 py-1 dark:border-white/10">2</button><button className="rounded-md border border-slate-200 px-2 py-1 dark:border-white/10">Sau</button></div></div>;
}
