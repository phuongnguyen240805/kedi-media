  ...
  return (
    <div className="relative">
      <button
        ...
        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-xs transition hover:border-kedi-yellow hover:text-kedi-navy dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
      >
        <CalendarDays className="h-4 w-4 text-kedi-navy" />
        {selectedLabel}
        ...
        <div className="absolute right-0 z-50 mt-2 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl dark:border-white/10 dark:bg-[#171923]">
          {presets.map((preset) => (
            <button
              ...
              className="flex w-full rounded-lg px-3 py-2 text-left text-xs font-medium text-slate-600 hover:bg-brand-50 hover:text-kedi-navy dark:text-slate-300 dark:hover:bg-kedi-yellow/10"
            >
              {preset.label}
            </button>
          ))}
