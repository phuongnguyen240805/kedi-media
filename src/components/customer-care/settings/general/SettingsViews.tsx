            ...
            {orderedDays.map((workingDay, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative border-r border-slate-300 p-2 text-left last:border-0 dark:border-white/15 ${workingDay.enabled ? "bg-kedi-yellow/[0.03]" : "bg-slate-100/50 dark:bg-white/[0.015]"}`}
                >
                  {workingDay.enabled && (
                    <div className="h-[336px] rounded-md border-l-4 border-kedi-yellow bg-kedi-yellow/[0.08] p-3 text-xs text-kedi-navy dark:text-kedi-yellow">
                      <div className="font-semibold">Lịch làm</div>
                      <div className="mt-1">{shift.from} - {shift.to}</div>
                      {index === 2 && <span className="absolute right-3 top-3 text-kedi-navy">×</span>}
                    </div>
                  )}
                </button>
              );
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(520px,1fr)]">
        ...
        <SettingsCard title="Thẻ hội thoại" subtitle={`${tags.length} thẻ`}>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4 dark:border-white/[0.08]">
            <div className="flex gap-5 text-sm font-medium"><button type="button" className="border-b-2 border-kedi-yellow pb-3 text-kedi-navy dark:text-kedi-yellow">Danh sách thẻ</button><button type="button" className="pb-3 text-slate-500">Thẻ ngừng sử dụng</button></div>
          </div>
          <div className="mb-4 flex flex-wrap gap-2">
            <label className="relative min-w-[220px] flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm kiếm thẻ" className="h-10 w-full rounded-lg border border-slate-300 bg-transparent pl-10 pr-3 text-sm outline-none focus:border-kedi-yellow dark:border-white/15" /></label>
...
export function AiSetting() {
  ...
  return (
    <div className="space-y-5">
      ...
      <SettingsCard>
        <div className="mb-5 rounded-lg border border-amber-400/60 bg-amber-400/15 px-4 py-3 text-sm leading-6 text-amber-800 dark:text-amber-200"><AlertCircle className="mr-2 inline h-4 w-4" />Để hoạt động chính xác hơn, AI cần được phép học từ các hội thoại chất lượng. <button type="button" className="font-semibold text-kedi-navy dark:text-kedi-yellow">Đào tạo ngay</button></div>
        <DetailConfigItem icon={<Sparkles />} title="Mô hình AI" description="Kedi cung cấp các tùy chọn AI khác nhau. Model thông minh hơn có chi phí cao hơn nhưng chất lượng phản hồi tốt hơn.">
          <div className="flex flex-wrap items-center gap-3">
            <input value={model} disabled={aiConfigLoading || aiConfigSaving} onChange={(event) => setModel(event.target.value)} onBlur={() => void saveAiConfig({ model: model.trim() || null })} placeholder="openrouter/openai/gpt-4o" className="h-10 min-w-[260px] rounded-lg border border-slate-300 bg-transparent px-3 text-sm outline-none focus:border-kedi-yellow disabled:opacity-60 dark:border-white/15" />
        ...
        <div className="mb-5 ml-0 overflow-hidden rounded-xl border border-slate-300 md:ml-12 dark:border-white/15"><div className="grid grid-cols-3 border-b border-slate-300 text-sm font-semibold dark:border-white/15"><div className="px-5 py-3">Hội thoại</div><div className="px-5 py-3">Files</div><button type="button" className="px-5 py-3 text-right text-kedi-navy dark:text-kedi-yellow">Tùy chỉnh</button></div><div className="grid grid-cols-3 px-5 py-4 text-sm"><div>0</div><div>0</div><div /></div></div>
        <DetailConfigItem icon={<CircleDollarSign />} title="Thanh toán" description="Phí sử dụng tính năng AI được trừ trực tiếp từ ví Kedi đã chọn.">
    ...
    <div className="space-y-5">
      ...
      <SettingsCard>
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-slate-300 pb-4 dark:border-white/15">
          <button type="button" className="border-b-2 border-kedi-yellow px-2 pb-4 text-base font-semibold text-kedi-navy dark:text-kedi-yellow">Trả lời nhanh</button>
  ...
  return (
    <div className="space-y-5">
      ...
      <SettingsCard title="Cài đặt giao diện">
        ...
        <div className="ml-0 mt-5 flex items-center gap-4 rounded-xl border-l-4 border-kedi-yellow bg-slate-50 p-4 md:ml-12 dark:bg-white/[0.04]">
  ...
  return (
    <div className="space-y-5">
      ...
      <SettingsCard className="!p-0">
        <div className="flex gap-7 overflow-x-auto border-b border-slate-200 px-6 dark:border-white/[0.08]">{tabs.map((tab) => <button type="button" key={tab.id} onClick={() => setActiveTab(tab.id)} className={`whitespace-nowrap border-b-2 px-1 py-4 text-sm font-semibold ${activeTab === tab.id ? "border-kedi-yellow text-kedi-navy dark:text-kedi-yellow" : "border-transparent text-slate-500"}`}>{tab.label}</button>)}</div>
      </SettingsCard>
      <SettingsCard>
        <div className="mb-5 flex flex-wrap justify-between gap-3"><label className="relative min-w-[240px]"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input placeholder="Tìm kiếm tên nhóm" className="h-10 w-full rounded-lg border border-slate-300 bg-transparent pl-10 pr-3 text-sm outline-none focus:border-kedi-yellow dark:border-white/15" /></label><SettingsButton onClick={() => setCreateModalOpen(true)}><Plus className="h-4 w-4" />Tạo nhóm đồng bộ</SettingsButton></div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {shippingProviders.map((provider) => {
          ...
          const inputClass = "h-10 w-full rounded-lg border border-slate-300 bg-transparent px-3 text-sm outline-none focus:border-kedi-yellow dark:border-white/15";
          return (
              ...
              <div className="mb-5 flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 text-base font-semibold text-slate-900 dark:text-white">
                    <PackageOpen className="h-5 w-5 text-kedi-navy" />
...
export function ToolSetting() {
  const toolItems = [
    { title: "Khóa truy cập API", description: "Tạo khóa bảo mật để tích hợp hệ thống bên ngoài với CSKH Kedi.", action: "Tạo khóa", icon: KeyRound },
    ...
    <div className="space-y-5">
      ...
      <SettingsCard>
        ...
        <div className="grid min-h-[480px] overflow-hidden rounded-xl border border-slate-300 lg:grid-cols-[minmax(0,1fr)_360px] dark:border-white/15">
          <div className="overflow-auto"><table className="w-full text-sm"><thead><tr className="border-b border-slate-300 dark:border-white/15"><th className="px-4 py-3 text-left">Người thao tác</th><th className="px-4 py-3 text-left">Hành động</th><th className="px-4 py-3 text-left">Đối tượng</th><th className="px-4 py-3 text-left">Thời gian</th></tr></thead><tbody>{settingHistoryItems.map((item) => <tr key={item.id} onClick={() => setSelectedHistoryId(item.id)} className={`cursor-pointer border-b border-slate-200 last:border-0 dark:border-white/[0.08] ${selectedHistoryId === item.id ? "bg-kedi-yellow/[0.08]" : "hover:bg-slate-50 dark:hover:bg-white/[0.03]"}`}><td className="px-4 py-4 font-medium">{item.actor}</td><td className="px-4 py-4">{item.action}</td><td className="px-4 py-4">{item.target}</td><td className="px-4 py-4 text-xs text-slate-500">{item.createdAt}</td></tr>)}</tbody></table></div>
          <aside className="border-t border-slate-300 p-5 lg:border-l lg:border-t-0 dark:border-white/15"><div className="text-sm font-semibold">Chi tiết thay đổi</div><div className="mt-5 space-y-4 text-sm"><div><div className="text-xs text-slate-500">Người thao tác</div><div className="mt-1 font-medium">{selectedHistory.actor}</div></div><div><div className="text-xs text-slate-500">Hành động</div><div className="mt-1 font-medium">{selectedHistory.action}</div></div><div><div className="text-xs text-slate-500">Cài đặt</div><div className="mt-1 font-medium">{selectedHistory.target}</div></div><div className="rounded-lg border border-kedi-yellow/20 bg-kedi-yellow/[0.08] p-3 text-xs leading-5 text-kedi-navy dark:text-kedi-yellow">Lịch sử chỉ dùng để đối chiếu. Một số thay đổi không thể hoàn tác trực tiếp.</div></div></aside>
        </div>
      </SettingsCard>
    </div>
  );
