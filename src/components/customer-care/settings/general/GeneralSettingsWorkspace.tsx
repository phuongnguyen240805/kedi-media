  ...
  return (
    <div className="flex h-full min-h-0 bg-[#f4f7f5] text-slate-900 dark:bg-[#1b211f] dark:text-slate-100">
      <aside className="hidden w-[236px] shrink-0 border-r border-slate-200 bg-white px-4 py-5 dark:border-white/[0.06] dark:bg-[#121715] lg:flex lg:flex-col">
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left text-sm transition ${
                  isActive
                    ? "bg-kedi-yellow/15 font-semibold text-kedi-navy dark:bg-white/[0.06] dark:text-kedi-yellow"
              ...
              >
                <Icon className={`h-[18px] w-[18px] ${isActive ? "text-kedi-navy dark:text-kedi-yellow" : "text-slate-400 group-hover:text-slate-700 dark:text-slate-300"}`} />
        ...
        <div className="mt-auto rounded-xl border border-kedi-yellow/40 bg-brand-50 p-3 dark:border-kedi-yellow/15 dark:bg-kedi-yellow/[0.05]">
          <div className="flex items-center gap-2 text-xs font-semibold text-kedi-navy dark:text-kedi-yellow"><ShieldCheck className="h-4 w-4" /> Cấu hình CSKH</div>
          <p className="mt-2 text-[10px] leading-4 text-kedi-navy/70 dark:text-kedi-yellow/60">Quản lý tập trung quy trình, tích hợp và quyền truy cập của đội ngũ.</p>
        </div>
      </aside>
              ...
              {settingNavigationItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    key={item.id}
                    onClick={() => handleSettingViewChange(item.id)}
                    className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${item.id === currentView ? "bg-kedi-yellow/15 text-kedi-navy dark:text-kedi-yellow" : "text-slate-600 dark:text-slate-300"}`}
                  >
