        ...
        <div className="flex h-[50px] items-center justify-between gap-2 px-3 sm:h-[46px] sm:gap-3">
          ...
          <div ref={menuRef} className="relative shrink-0">
            <button
              ...
              className="flex h-9 max-w-[148px] items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 text-[11px] font-semibold text-slate-600 transition hover:border-kedi-yellow hover:bg-white sm:h-8 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"
            ...
            {channelMenuOpen ? (
              <div className="absolute right-0 top-[42px] z-[90] w-[min(15rem,calc(100vw-1rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-2xl sm:top-[38px] dark:border-white/10 dark:bg-[#171b25]">
                <button
                  ...
                  className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-sm transition ${selectedChannels.length === 0 ? "bg-brand-50 text-kedi-navy dark:bg-kedi-yellow/10 dark:text-kedi-yellow" : "text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-white/[0.06]"}`}
                >
        ...
        <div className="flex h-[48px] items-center gap-2 px-2 pb-2 sm:h-[42px]">
          ...
          <label className="relative min-w-0 flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-slate-200 bg-slate-50 pl-8 pr-3 text-base text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-kedi-yellow focus:bg-white focus:ring-2 focus:ring-kedi-yellow/10 sm:h-8 sm:text-xs dark:border-white/10 dark:bg-white/5 dark:text-white"
            />
          </label>
        </div>
  ...
  return (
    <div
      className={`group relative min-h-[76px] w-full border-b transition-colors dark:border-white/[0.07] ${
        active
          ? "border-kedi-yellow/40 bg-brand-50/80 dark:border-kedi-yellow/20 dark:bg-kedi-yellow/[0.09]"
          : "border-slate-100 hover:bg-slate-50 dark:hover:bg-white/[0.035]"
      }`}
    >
      {active ? (
        <span className="pointer-events-none absolute inset-y-0 left-0 z-10 w-[3px] bg-kedi-yellow" />
      ) : null}
