...
export function CustomerDetailPanel({ conversation, open, onClose, width }: {
  ...
  return (
    ...
    >
        ...
        <div className="mt-3 space-y-3">
          <ControlField label="Nhân viên">
            <select
              ...
              className="h-11 w-full rounded-lg border border-slate-200 bg-slate-50 px-2 text-base text-slate-700 outline-none focus:border-kedi-yellow disabled:opacity-60 sm:h-9 sm:text-xs dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
            >
      ...
      <section className="border-b border-slate-100 p-4 dark:border-white/[0.07]">
        <SectionTitle icon={<CalendarClock className="h-4 w-4" />} title="Hội thoại liên quan" action={<button type="button" onClick={() => void previousQuery.refetch()} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-kedi-navy dark:hover:bg-white/5"><RefreshCw className="h-3.5 w-3.5" /></button>} />
        <div className="mt-3 space-y-2">
          {previousQuery.data?.length ? previousQuery.data.slice(0, 5).map((item) => (
            <a key={item.id} href={`/cskh/hoi-thoai?conversationId=${item.id}`} className="flex items-center gap-2 rounded-xl border border-slate-100 p-2.5 text-xs hover:border-kedi-yellow hover:bg-brand-50/50 dark:border-white/[0.07] dark:hover:bg-kedi-yellow/5">
        ...
        {customerOrders.length ? <div className="mt-3 space-y-2">
          {customerOrders.slice(0, 5).map((order) => <div key={order.orderId} className="rounded-xl border border-slate-200 p-3 text-left dark:border-white/10">
            ...
            {order.shipment ? <div className="mt-2 rounded-lg bg-slate-50 px-2.5 py-2 text-[10px] dark:bg-white/5">
              <div className="flex items-center justify-between gap-2"><span className="font-semibold uppercase text-kedi-navy dark:text-kedi-yellow">{order.shipment.provider.replaceAll("_", " ")}</span><span>{order.shipment.status}</span></div>
          ...
          <div className="mt-1 text-[10px] leading-4 text-slate-400">Khách hàng lấy từ CRM và sản phẩm lấy trực tiếp từ kho sản phẩm Kedi.</div>
        </div>}
        <button type="button" onClick={() => setOrderModalOpen(true)} className="mt-3 inline-flex h-11 w-full items-center justify-center gap-1 rounded-lg border border-kedi-yellow px-3 text-xs font-semibold text-kedi-navy transition hover:bg-brand-50 sm:h-8 dark:text-kedi-yellow dark:hover:bg-kedi-yellow/10"><Plus className="h-3.5 w-3.5" /> Tạo đơn hàng</button>
      </section>
